...
#
# Usage:
#  hpe-service-build.sh -s <service-template> -o <service_yml_filename>
...
if [ -z "$OS_TEMPLATE_FILE" -o -z "$SVC_CATEGORY" -o -z "$SVC_FLAVOR" -o \
     -z "$SVC_VER" -o -z "$SERVICE_YML_FILENAME" -o -z "$DISPLAY_URL" -o \
     -z "$SECURE_URL" ]; then
cat << EOF
